...
{ 
	namespace shape
	{
		...
		bool 	buildCube24 (const vec3f &center, const vec3f &size, gos::Allocator *allocator, Shape *out);
		...
		bool 	buildSphere (const vec3f &center, const vec3f &radius, u32 numPointPerCirconferenza, u32 numHalfStack, gos::Allocator *allocator, Shape *out);
		
		//buildCylinder
		//La base e' centrata in [center] e sale verso l'alto. Crea [numStack] circonferenze (compresa quella di base)
		// In base a come e' stato definito [shape->vtxLayout], filla "pos" e/o "norm"
		bool 	buildCylinder (const vec3f &center, f32 radius, f32 height, u32 numPointPerCirconferenza, u32 numStack, bool bCloseTop, bool bCloseBottom, gos::Allocator *allocator, Shape *out);
