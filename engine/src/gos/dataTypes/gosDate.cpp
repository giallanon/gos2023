...
//****************************
void Date::formatAs_YYYYMMDD(char *out, u32 sizeof_out, char date_sep) const
{
	if (date_sep == 0x00)
	{
		assert(sizeof_out >= 9);
		if (sizeof_out >= 9)
			sprintf_s(out, sizeof_out, "%04d%02d%02d", getYear(), getMonth(), getDay());
		else
			out[0] = 0;
	}
	else
	{
		assert(sizeof_out >= 11);
		if (sizeof_out >= 11)
			sprintf_s(out, sizeof_out, "%04d%c%02d%c%02d", getYear(), date_sep, getMonth(), date_sep, getDay());
		else
			out[0] = 0;
	}
}
