...
bool mem::priv_init (const gos::sGOSInit &init)
{
	...
	systemHeapAllocator->setup (init._memory.startingSizeOfDefaultHeapAllocator_MB * 1024 * 1024);

	systemScrapAllocator = new GOSMemGlobalHeapAllocatorTS("sysScrap");
	systemScrapAllocator->setup (init._memory.startingSizeOfScrapAllocator_MB * 1024 * 1024);
