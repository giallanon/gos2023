 ...
 * I piani sono definiti in modo da avere le normali che puntano all'interno 
   del frustum*/
void Frustum3::build_perspective (const vec3f &o, const vec3f &asseZ, const vec3f &asseY, const vec3f &asseX, f32 nearDistance, f32 farDistanceIN, f32 fovY_rad, f32 aspect)
{
	...
	planes[(u8)ePlane::left].set_from_3points (p4near[0], p4far[0], p4far[3]);
	planes[(u8)ePlane::right].set_from_3points (p4far[2], p4far[1], p4near[1]);
	planes[(u8)ePlane::top].set_from_3points (p4near[0], p4near[1], p4far[1]);
	planes[(u8)ePlane::bottom].set_from_3points (p4near[2], p4near[3], p4far[3]);
 ...
 * I piani sono definiti in modo da avere le normali che puntano all'interno 
   del frustum*/
void Frustum3::build_ortho (const vec3f &o, const vec3f &asseZ, const vec3f &asseY, const vec3f &asseX, f32 width, f32 height, f32 nearDistance, f32 farDistanceIN, f32 zoom)
{
	farDistance = farDistanceIN;
	assert (zoom > 0);
	width/=zoom;
	...
	planes[(u8)ePlane::left].set_from_point_and_normal		(p -farPlaneHalfAsseX,  asseX);
	planes[(u8)ePlane::right].set_from_point_and_normal	(p +farPlaneHalfAsseX, -asseX);
	planes[(u8)ePlane::top].set_from_point_and_normal		(p +farPlaneHalfAsseY, -asseY);
	planes[(u8)ePlane::bottom].set_from_point_and_normal	(p -farPlaneHalfAsseY,  asseY);
...
void Frustum3::calc_8points (vec3f *vtx) const
{
	...
	vec3f	nearAy = get_nearPlane_half_asseY();
...
//************************************************************************
void Frustum3::calc_AABB (AABB3 *out) const
{
	vec3f pt[8];
	calc_8points (pt);
