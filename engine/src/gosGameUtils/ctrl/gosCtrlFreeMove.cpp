#include "gosCtrlFreeMove.h"

using namespace gos;

//*******************************
CtrlFreeMove::CtrlFreeMove ()
{
...
//*******************************
void CtrlFreeMove::bind (geom::Pos3 *posIN)
{
	pos = posIN;
...
//*******************************
void CtrlFreeMove::update (u64 timenow_msec, CtrlAction &ctrl)
{
	if (NULL == pos)
		return;
	...
	const eCtrlActionBitmask bm = ctrl.update_begin();
	{
		...
		if (rot_x__rad < -gos::math::PIMEZZI)		rot_x__rad = -gos::math::PIMEZZI;
		else if (rot_x__rad > gos::math::PIMEZZI)	rot_x__rad = gos::math::PIMEZZI;
