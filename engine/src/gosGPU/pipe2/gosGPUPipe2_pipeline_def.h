...
namespace gos
{
    ...
    namespace gpu
    {
        ...
        {
        public:
            static constexpr u8 ZBUFFER_FLAG__ENABLED                           = 0;
            static constexpr u8 ZBUFFER_FLAG__ZWRITE_ENABLED                    = 1;
            static constexpr u8 ZBUFFER_FLAG__ALLOW_DEPTH_TEST_ENABLE_DISABLE   = 2;
            static constexpr u8 ZBUFFER_FLAG__ALLOW_DEPTH_WRITE_ENABLE_DISABLE  = 3;
        ...
        public:
            ...
            Pipeline_def&   zbuffer_define (eImageFormat fmt, bool zwriteIN=true, eZFunc zfuncIN=eZFunc::LESS)
            { 
                assert (gos::utils::isFormatWithDepth(fmt) || fmt==eImageFormat::_DEPTH_BEST); 
                zbuffer_flag.set(ZBUFFER_FLAG__ENABLED);
                zbuffer_format=fmt; 
                if (zwriteIN) zbuffer_flag.set(ZBUFFER_FLAG__ZWRITE_ENABLED);
                zbuffer_cmpFn=zfuncIN; 
                return *this; 
            }
            Pipeline_def&   zbuffer_allow_depthTestEnablingDisabling()                                          { zbuffer_flag.set(ZBUFFER_FLAG__ALLOW_DEPTH_TEST_ENABLE_DISABLE); return *this; }
            Pipeline_def&   zbuffer_allow_depthWriteEnablingDisabling()                                         { zbuffer_flag.set(ZBUFFER_FLAG__ALLOW_DEPTH_WRITE_ENABLE_DISABLE); return *this; }

            Pipeline_def&   rt_add (eImageFormat fmt)                                                           { assert (numRT < GOSGPU__NUM_MAX_ATTACHMENT); renderTargetFormat[numRT] = fmt; numRT++; return *this; }
            ...
            //query
            bool    zbuffer_is_enabled() const                                  { return zbuffer_flag.isBitSet(ZBUFFER_FLAG__ENABLED); }
            bool    zbuffer_is_write_enabled() const                            { return zbuffer_flag.isBitSet(ZBUFFER_FLAG__ZWRITE_ENABLED); }
            bool    zbuffer_is_depthTestEnablingDisabling_enabled() const       { return zbuffer_flag.isBitSet(ZBUFFER_FLAG__ALLOW_DEPTH_TEST_ENABLE_DISABLE); }
            bool    zbuffer_is_depthWriteEnablingDisabling_enabled() const      { return zbuffer_flag.isBitSet(ZBUFFER_FLAG__ALLOW_DEPTH_WRITE_ENABLE_DISABLE); }
