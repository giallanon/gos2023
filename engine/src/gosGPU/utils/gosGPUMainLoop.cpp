...
bool AquireSwapChainImage::tryAcquire ()
{
    ...
    {
        if (!gpu->swapChain_acquireImage_ex (&acquiredImg.index, 0, VK_NULL_HANDLE, fence))
            return false;
    ...
    if (eStato::waitingFence == stato)
    {
        ...
        acquiredImg.image = gpu->swapChain_getImageByIndex(acquiredImg.index);
        return true;
    }
...
bool MainLoop2::gfxJob_canSubmit (AcquiredSwapchainImg *out)
{
...
void MainLoop2::gfxJob_submitAndPresent (const GPUCmdBufferHandle &cmdBufferHandle, const AcquiredSwapchainImg &info)
{
    gfxJob.submit (cmdBufferHandle, info.index);
}