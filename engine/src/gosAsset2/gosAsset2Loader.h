#ifndef _gosAsset2Loader_h_
#define _gosAsset2Loader_h_
#include "loaders/gosAsset2Loader_shader.h"
#include "loaders/gosAsset2Loader_pipe.h"
#include "loaders/gosAsset2Loader_tex2D.h"
    ...
    namespace asset2
    {
        ...
        */
        ...
        public:
            ...
            bool    setup (const char *baseFolder, gos::GPU *gpu, asset2::Hub *theHub);
            ...
            bool    load (const UID &uid, void *ptToAssetData)
            {
            ...
            void    unload (const UID &uid, void *ptToAssetData)
            {
                const eAssetType assType = uid.getAssetType();
                LoaderInterface *loader = getLoader (assType);
                if (loader)
            ...
            bool    load (const UID &uid, TASSET *ptToAssetData)
            {
                const eAssetType assType = uid.getAssetType();
                LoaderInterface *loader = getLoader (assType);
                if (NULL == loader)
                ...
                gos::logger::err ("asset2::Loader::Load (%016" PRIX64 ") => asset not found\n", uid._uid);
                return false;
            }
            ...
            bool    load (const char *runtimeName, TASSET *ptToAssetData)
            {
                ...
                {
                    gos::logger::err ("asset2::Loader::Load('%s') => asset not found\n", runtimeName);
                    return false;
                }
            ...
            bool                runtimeNameToUID (const char *runtimeName, UID *out);
            DBContext*          getContext()                                                        { return &ctx;}
            asset2::Hub*        getTheHub() const                                                   { return theHub; }
