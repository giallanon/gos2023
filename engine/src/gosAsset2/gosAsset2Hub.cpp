...
void Hub::priv_free ()
{
    ...
    knownAssetsList.forEach ([a] (UID uid, void *pt) 
    {
        GOSFREE(a, pt);
        return true;
    });
...
bool Hub::setup (const char *baseFolderIN, gos::GPU *gpuIN)
{
    ...
    {
        logger->err ("asset2::Hub::setup() => error initializing loader in '%s'\n", baseFolderIN);
        return false;
    }
    ...
    {
        logger->err ("asset2::Hub::setup() => error creating thread: errcode=%d\n", static_cast<int>(err));
        return false;
    }

    if (!thread::eventWait (params.hEvent_started, 20000))
    {
        logger->err ("asset2::Hub::setup() => error waiting for thread to start\n");
        return false;
    }
...
//***************************************
void Hub::priv_update (u64 timeout_msec)
{
 ...
 */
bool Hub::priv_findOrAddAsset (const UID &uid, void **out_pt)
{
    ...
    asset2::LoaderInterface *l = loader.getLoader( uid.getAssetType());
    if (NULL == l)
    {
        gos::logger::err ("asset2::Hub::priv_findOrAddAsset() => can't find a loader for resource %s\n", asset2::enumToString(uid.getAssetType()));
        *out_pt = NULL;
        return false;
    }    
...
//***************************************
void* Hub::priv_getExistingAssetByUID (const UID &uid)
{
    void *pt = NULL;
...
bool Hub::getHandle (const char *runtimeName, Handle *out, bool bScheduleLoadNow )
{
    ...
    {
        gos::logger::err ("asset2::Hub::getHandle() => invalid runtineName '%s'\n", runtimeName);
        return false;
    }
...
void Hub::priv_scheduleLoad (void *pt)
{
    ...
    //gli altri
    u8 memblock[1024];
    FastUIDList fastUIDList;
    fastUIDList.setupWithBase (memblock, sizeof(memblock), gos::getScrapAllocator());

    asset2::asset_get_runtime_dependecies_list (*loader.getContext(), header->uid, true, &fastUIDList);
...
void Hub::priv_unload (void *pt)
{
    ...
    //intanto inizio a unloadare me stesso
