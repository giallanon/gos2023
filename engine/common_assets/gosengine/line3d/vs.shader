...
void main()
{
    ...
    if (0xFFFF0000 == (packed_idx_1_2 & 0xFFFF0000))
    {
        //E' un punto

        //centro del punto in world-pos
        const vec3 world_point_center = vec3 (vtxList.pos[idx2], vtxList.pos[idx2+1], vtxList.pos[idx2+2]);

        //..in clip pos
        const vec4 clip_point_center = vec4(world_point_center, 1.0) * scene.camVP;

        //.. in screen pos
        const vec2 screen_point_center = ((clip_point_center.xy / clip_point_center.w) + 1) * 0.5 * scene.screen_wh.xy;

        //in_position.x vale 0 se il vertice e' all'inizio del segmento, vale 1 se e' alla fine del segmento
        vec2 p0 = screen_point_center + (pc.point_radius * (in_position.x - 0.5)) * vec2(1,0) + (pc.point_radius * in_position.y)  * vec2(0,1);

        //passo al PS il vertice in clip space
        gl_Position = vec4(
            clip_point_center.w *  ( ( (p0.xy / scene.screen_wh.xy) * 2.0) - 1.0),
            clip_point_center.z,
            clip_point_center.w
        );
    }
